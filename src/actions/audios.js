...
export function add(audioID, userID, albumID) {
  return (dispatch) =>
    Audio.add(audioID, userID, albumID).then(() => dispatch(addSuccess()));
}

export function remove(audioID, userID) {
  return (dispatch) =>
    Audio.remove(audioID, userID).then(() => dispatch(removeSuccess()))
}
...
export function fetchRecomended(userID, offset = 0, count = 100) {
  return (dispatch) => {
    ...
    return Audio.getRecommendations(userID, offset, count).then(({items, count}) => {
      dispatch(load({id: 'recomended', title: 'Recomended', items, count, offset: offset + items.length}));
      dispatch(menuHide());
...
export function fetchWall(userID, offset = 0, count = 100) {
  return (dispatch) => {
    ...
    return Audio.getWall(userID, offset, count).then((items) => {
      dispatch(load({id: 'wall', title: 'Wall', items, count: items.length, offset: offset + items.length}));
      dispatch(menuHide());
    }, (err) => dispatch(error(err)));
  };
}
...
export function fetchPopular(userID, offset = 0, count = 100) {
  return (dispatch) => {
    ...
    return Audio.getPopular(userID, offset, count).then((items) => {
      dispatch(load({id: 'popular', title: 'Popular', items, count: items.length, offset: offset + items.length}));
      dispatch(menuHide());
    }, (err) => dispatch(error(err)));
  };
}
...
export function fetchAll(userID, offset = 0, count = 100) {
  return (dispatch) => {
    ...
    return Audio.getAll(userID, offset, count).then(({items, count}) => {
      dispatch(load({id: 'all', title: 'All', items, count, offset: offset + items.length}));
      dispatch(menuHide());
    }, (err) => dispatch(error(err)))
  };
}
...
export function fetchAlbum(userID, id, title, offset = 0, count = 100) {
  return (dispatch) => {
    ...
    return Audio.get(userID, id, offset, count).then(({items, count}) => {
      dispatch(load({id, title, items, count, offset: offset + items.length}));
      dispatch(menuHide());
    }, (err) => dispatch(error(err)))
  };
}
...
export function fetchGroup(id, title, offset = 0, count = 100) {
  return (dispatch) => {
    ...
    return Audio.get(id, null, offset, count).then(({items, count}) => {
      dispatch(load({id, title, items, count, offset}));
      dispatch(menuHide());
    }, (err) => dispatch(error(err)))
  };
}
